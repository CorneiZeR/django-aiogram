...
| `tg_bot_id` | which bot the line is about, by the number in its token. On every send, delivery, quarantine and reconciliation line since 5.0 — a deployment with twenty clients cannot read a line that does not say whose bot it was. In `extra` and never interpolated into the message: a line that carried it in the text would be its own string, ungreppable and unusable as a field to filter on |
...
| `taking over a replay claim whose queue write never answered` | WARNING | a claim older than `--claim-lease` that never recorded reaching the queue: the run that made it died, or its `publish` raised. `tg_claimed_by` names the process whose claim was taken over, `tg_replay_of` the failure. Taking it over may send a second copy, since the message may have reached the queue anyway |
| `a replay claim was taken over while its message was being queued` | WARNING | the queue write outlasted `--claim-lease`, so another run took the failure and may have sent the same message. `tg_replay_of` names it, `tg_claimed_by` this run's own process, which held the claim that was lost. The message this run queued still went, and the run carries on: the alternative is ending it at that row |
| `could not replay a failed send` | ERROR | one row's queue write raised, and the run carried on with the rest. `tg_replay_of` names it and `tg_claimed_by` the process holding the claim it keeps — a raise is not proof the message stayed out of the queue, so the row waits for `--claim-lease` rather than being offered to the next run. One bad row must not take a hundred-row replay down halfway, with no way to tell which half went |
