...
| `bot.cancel_scheduled(correlation_id)` | delete the rows this id still has waiting; returns how many |

Every form that *queues* takes `eta=<aware datetime>` — `send`, `enqueue`, `send_many` and
