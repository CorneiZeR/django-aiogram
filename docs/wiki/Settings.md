...
| `METRICS_PER_BOT` | `False` | The process's, not a bot's. Whether the shipped Prometheus exporter labels its series by bot. Off, because a label per bot is a series per bot **per kind**: a thousand clients turn two metrics into fourteen thousand series, which is a Prometheus problem rather than a dashboard. The event log answers the same question per bot and is queryable — see **[Event log](Event-log.md)** |
