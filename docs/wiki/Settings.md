...
| `E029` | `WEBHOOK_ALLOWED_UPDATES` is not a list, tuple or set of update types, or names one Telegram does not have. A mapping is refused although it is a collection: it is a collection *of its keys*, so it would register those and drop the values silently |
...
| `E032`, `E035` | `EVENT_LOG_KINDS` / `EVENT_LOG_REDACT_KEYS` is not a list, tuple or set of strings. A mapping is refused for the reason `E029` gives |
