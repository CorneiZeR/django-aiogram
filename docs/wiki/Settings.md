...
a token is a credential it may rotate. The settings in *Which processes run the bot* and
every `EVENT_LOG_*` one belong to the process rather than to a bot, so a section naming one
is refused by `E053` instead of deciding for every bot in the process.
...
| --- | --- | --- |
| `django_aiogram.broker.redis_list.RedisListBroker` | `REDIS_URL`, `REDIS_MESSAGES_KEY`, `REDIS_TIMEOUT` | none — the list has a default name |
| `django_aiogram.broker.redis_streams.RedisStreamsBroker` | `REDIS_URL`, `REDIS_STREAM_KEY`, `REDIS_STREAM_GROUP`, `REDIS_TIMEOUT` | **`REDIS_STREAM_KEY`** |
...
The Redis list declares three settings, gathered here because the other three transports have
their own sections.

Two of them are in the package-wide tables above as well, and mean it: `REDIS_URL` under
**Credentials** and `REDIS_TIMEOUT` under **Queue** are read by the package under *every*
transport, because the FSM storage builds a Redis client whatever carries the messages. A project
running Kafka with `FSM_STORAGE = 'redis'` sets both and neither is stranded. `REDIS_MESSAGES_KEY`
is not like that — a list key is read by nothing under Kafka — so it lives here alone, `W003`
reports it where another transport is configured, and `E007` validates it only where this one is.
...
| `EVENT_LOG_KINDS` | `()` | Which kinds to keep; empty means every kind this version knows. Naming any also opts out of the kinds a later release adds, and filters `events_recorded` receivers as well as rows. `log.dropped` is exempt either way — it is the record that recording fell behind. A set that leaves out any of the four kinds a correct outcome requires also switches off `bot.outcome()`, and the call refuses rather than answering `unknown` for ever — **[Event log](Event-log.md#what-became-of-one-message)** has which four and why |
...
| `E050` | `TELEGRAM_BOT` is still set. 5.0 split it into `TELEGRAM_BOT_DEFAULTS` and `TELEGRAM_BOTS`, and nothing reads the old name — a project that kept it would run on defaults alone, with no token and no transport |
...
| `E053` | a bot's section sets a setting the process owns — `AUTODISCOVER`, `MODULE_NAME`, `WORKER_NAME` or an `EVENT_LOG_*` one. There is one writer thread and one in-flight list per process, so a per-bot value could only mean whichever bot resolved last wins |
