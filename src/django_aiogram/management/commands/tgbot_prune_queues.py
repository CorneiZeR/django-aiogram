...
class Command(BaseCommand):
    ...
    def _act_on(self, row: 'TelegramQueue', policy: RemovedQueuePolicy, *, dry_run: bool) -> None:
        ...
        if policy is RemovedQueuePolicy.PARK:
            self.stdout.write(f'{row.name}: {depth} message(s), parked — nothing publishes to it any more.')
            return
        if policy is RemovedQueuePolicy.HOLD and depth:
            self.stdout.write(f'{row.name}: {depth} message(s), held — it is not empty yet.')
            return
        if dry_run:
            self.stdout.write(f'{row.name}: {depth} message(s), would be removed.')
            return
        if not broker.discard():
            # not a failure: this transport does not remove its own queues -- Kafka, where a
        ...
        self.stdout.write(f'{row.name}: removed, with {depth} message(s).')
